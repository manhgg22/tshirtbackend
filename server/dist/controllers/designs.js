"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.getPublicDesigns = exports.getMyDesigns = exports.createDesign = void 0;
const Design_1 = __importDefault(require("../models/Design"));
const createDesign = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b;
    try {
        const { name, isPublic } = req.body;
        const image = (_a = req.file) === null || _a === void 0 ? void 0 : _a.filename;
        const userId = (_b = req.user) === null || _b === void 0 ? void 0 : _b.userId;
        if (!image || !userId) {
            return res.status(400).json({ message: 'Image and user authentication required' });
        }
        const design = new Design_1.default({
            userId,
            name,
            image: `/uploads/${image}`,
            isPublic,
        });
        yield design.save();
        res.status(201).json(design);
    }
    catch (error) {
        res.status(500).json({ message: 'Error creating design' });
    }
});
exports.createDesign = createDesign;
const getMyDesigns = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        const userId = (_a = req.user) === null || _a === void 0 ? void 0 : _a.userId;
        const designs = yield Design_1.default.find({ userId });
        res.json(designs);
    }
    catch (error) {
        res.status(500).json({ message: 'Error fetching designs' });
    }
});
exports.getMyDesigns = getMyDesigns;
const getPublicDesigns = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const designs = yield Design_1.default.find({ isPublic: true });
        res.json(designs);
    }
    catch (error) {
        res.status(500).json({ message: 'Error fetching public designs' });
    }
});
exports.getPublicDesigns = getPublicDesigns;
